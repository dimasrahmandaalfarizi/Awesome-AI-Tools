...
program.name("awesome-ai-tools").description("CLI to manage 400+ AI skills, 68+ AI subagents, and run AgentShield security auditing across AI IDEs and CLI harnesses").version("0.5.0");
function toCommandName(slug) {
  return slug.replace(/^skill-/, "").replace(/[^a-zA-Z0-9_-]/g, "-").toLowerCase();
}
program.command("list").description("List all available AI skills and subagents").action(() => {
  console.log("\n\u{1F680} Available AI Agent Skills (413 Skills):\n");
...
program.command("scan").description("Run AgentShield security audit on current workspace for secrets, prompt injection, and dangerous hooks").option("-d, --dir <path>", "Target directory to scan", ".").action((options) => {
  const targetDir = path2.resolve(process.cwd(), options.dir);
  console.log(`
...
`);
  ...
  console.log("\u{1F4A1} Tip: Resolve critical leaks and protect prompt files before deploying autonomous loops.\n");
});
program.command("init").description("Install all AI skills & subagents into your favorite IDE or CLI harness").option("-t, --target <type>", "Target IDE/Platform (all, cursor, antigravity, claude, continue, copilot, windsurf, cline)").action(async (options) => {
  ...
  if (!target) {
    const response = await prompts({
      ...
      choices: [
        {
          title: "\u{1F31F} All-in-One Universal Suite (CLI + Cursor + Antigravity + Copilot + Continue + Windsurf + Subagents)",
          value: "all",
...
`);
  if (target === "all" || target === "claude") {
    const claudeCmdDir = path2.join(cwd, ".claude", "commands");
    if (!fs2.existsSync(claudeCmdDir)) fs2.mkdirSync(claudeCmdDir, { recursive: true });
    AI_SKILLS.forEach((skill) => {
      const cmdName = toCommandName(skill.slug);
      const filePath = path2.join(claudeCmdDir, `${cmdName}.md`);
...
`;
  ...
  if (target === "all" || target === "cursor") {
    ...
    if (!fs2.existsSync(cursorRulesDir)) fs2.mkdirSync(cursorRulesDir, { recursive: true });
    AI_SKILLS.forEach((skill) => {
      const cmdName = toCommandName(skill.slug);
      const filePath = path2.join(cursorRulesDir, `${cmdName}.mdc`);
      const fileContent = `---
...
`;
  ...
  if (target === "all" || target === "antigravity") {
    ...
    AI_SKILLS.forEach((skill) => {
      const skillDir = path2.join(agentsSkillsDir, skill.slug);
      if (!fs2.existsSync(skillDir)) fs2.mkdirSync(skillDir, { recursive: true });
...
`;
    ...
    const subagentsDir = path2.join(cwd, ".agents", "subagents");
    if (!fs2.existsSync(subagentsDir)) fs2.mkdirSync(subagentsDir, { recursive: true });
    AI_AGENTS.forEach((agent) => {
      const filePath = path2.join(subagentsDir, `${agent.slug}.md`);
...
`;
  ...
  if (target === "all" || target === "continue") {
    const continuePromptsDir = path2.join(cwd, ".continue", "prompts");
    if (!fs2.existsSync(continuePromptsDir)) fs2.mkdirSync(continuePromptsDir, { recursive: true });
    AI_SKILLS.forEach((skill) => {
      const cmdName = toCommandName(skill.slug);
      const filePath = path2.join(continuePromptsDir, `${cmdName}.prompt`);
...
`;
  ...
  if (target === "all" || target === "copilot") {
    const copilotPromptsDir = path2.join(cwd, ".github", "prompts");
    if (!fs2.existsSync(copilotPromptsDir)) fs2.mkdirSync(copilotPromptsDir, { recursive: true });
    AI_SKILLS.forEach((skill) => {
      const cmdName = toCommandName(skill.slug);
      const filePath = path2.join(copilotPromptsDir, `${cmdName}.prompt.md`);
      const fileContent = `---
name: ${cmdName}
description: ${skill.description}
---
...
`;
    ...
    const instructionsPath = path2.join(cwd, ".github", "copilot-instructions.md");
...
`;
    fs2.writeFileSync(instructionsPath, instructionsContent, "utf8");
  ...
  if (target === "all" || target === "windsurf") {
    const windsurfDir = path2.join(cwd, ".windsurf", "workflows");
    if (!fs2.existsSync(windsurfDir)) fs2.mkdirSync(windsurfDir, { recursive: true });
    AI_SKILLS.forEach((skill) => {
      const cmdName = toCommandName(skill.slug);
      const filePath = path2.join(windsurfDir, `${cmdName}.md`);
...
`;
  fs2.writeFileSync(agentsMdPath, agentsMdContent, "utf8");
