...
program.command("learn").description("Persist a permanent codebase rule or preference to instincts.md").argument("<rule>", "The rule, architecture decision, or invariant to remember").action((rule) => {
  ...
  const instinctsPath = path2.join(cwd, "instincts.md");
  if (!fs2.existsSync(instinctsPath)) {
    fs2.writeFileSync(instinctsPath, INSTINCTS_TEMPLATE, "utf8");
  }
  const timestamp = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  const newEntry = `
- [${timestamp}] ${rule}
`;
  fs2.appendFileSync(instinctsPath, newEntry, "utf8");
  console.log(`
...
`);
  ...
  if (target === "all" || target === "claude" || target === "antigravity") {
    const claudeHooksDir = path2.join(cwd, ".claude", "hooks");
    if (!fs2.existsSync(claudeHooksDir)) fs2.mkdirSync(claudeHooksDir, { recursive: true });
    fs2.writeFileSync(path2.join(claudeHooksDir, "pre-tool-call.js"), HOOK_SCRIPTS.preToolCall, "utf8");
    fs2.writeFileSync(path2.join(claudeHooksDir, "post-tool-call.js"), HOOK_SCRIPTS.postToolCall, "utf8");
    fs2.writeFileSync(path2.join(claudeHooksDir, "on-session-end.js"), HOOK_SCRIPTS.onSessionEnd, "utf8");
  ...
  if (target === "all" || target === "claude" || target === "continue") {
    ...
    COMPOSITE_WORKFLOWS.forEach((wf) => {
      const filePath = path2.join(claudeCmdDir, `${wf.slug}.md`);
      fs2.writeFileSync(filePath, wf.content, "utf8");
    });
